...
// ==========================================
const _cockpitDefaults = {
    frontBack:      -2.50,   // new default (was 14.0)
    upDown:          0.80,   // new default (was 4.5)
    sidePosition:   -0.50,   // new default (was -0.8)
    horizontalTurn:  0.0,
    verticalTurn:   -7.0,    // new default (was 0.0)
    fovOffset:      10.0     // new default (was 0.0)
};
(function _loadCockpitSettings() {
    const saved = localStorage.getItem('biv_cockpit_cam');
    if (saved) {
        try { Object.assign(_cockpitDefaults, JSON.parse(saved)); } catch(e) {}
    }
})();
const cockpitCam = Object.assign({}, _cockpitDefaults);
    ...
    if (settings.cesiumCameraMode === 'third') {
        ...
        if (isPlane) {
            ...
            const COCKPIT_OFFSET = new Cesium.Cartesian3(
                cockpitCam.sidePosition,
                cockpitCam.frontBack,
                cockpitCam.upDown
            );
        ...
        cesiumViewer.camera.setView({
            ...
            orientation: {
                heading: Cesium.Math.toRadians(state.heading  + cockpitCam.horizontalTurn + freeLook.yaw),
                pitch:   Cesium.Math.toRadians(camPitchDeg   + (isPlane ? cockpitCam.verticalTurn : 0) + freeLook.pitch),
                roll:    camRoll
            }
        });
        ...
        if (cesiumViewer.scene.camera.frustum &&
            ...
            cesiumViewer.scene.camera.frustum.fov =
                Cesium.Math.toRadians(settings.fov + (isPlane ? cockpitCam.fovOffset : 0));
        }
    }
}
...
/** Increment or decrement one cockpit cam parameter, then refresh its display. */
function adjCockpit(key, delta) {
    const m = _ccamMeta[key];
    cockpitCam[key] = parseFloat((cockpitCam[key] + delta).toFixed(m.decimals));
    _ccamRefreshOne(key);
}
...
function loadLastSaveCockpitCam() {
    ...
    if (!raw) return;
    try { Object.assign(cockpitCam, JSON.parse(raw)); refreshCockpitCamVals(); } catch(e) {}
}

function resetCockpitCam() {
    Object.assign(cockpitCam, {
        frontBack:      -2.50,
        upDown:          0.80,
        sidePosition:   -0.50,
        horizontalTurn:  0.0,
...
function _ccamRefreshOne(key) {
    ...
    if (!el) return;
    const m = _ccamMeta[key];
    el.textContent = cockpitCam[key].toFixed(m.decimals) + m.unit;
}
