...
function tryLoadOsmBuildings() {
    ...
    Promise.resolve(buildingPromise)
        .then(tileset => {
            ...
            applyOsmBuildingsSatelliteTexture(tileset);
 ...
 *
 * Requires Cesium ≥1.90 (CustomShader + TextureUniform API).
 */
function applyOsmBuildingsSatelliteTexture(tileset) {
    if (!Cesium.CustomShader || !Cesium.TextureUniform) {
        console.warn('Satellite building texture requires Cesium ≥1.90 (CustomShader API) — skipped.');
        return;
    }

    const zoom    = 16;  // ~1.2 km per tile — good coverage for a city-block view
    ...
    tileset.customShader = new Cesium.CustomShader({
        uniforms: {
            u_sat: {
                ...
                value: new Cesium.TextureUniform({ url: tileUrl })
            },
        ...
        fragmentShaderText: `
            void fragmentMain(FragmentInput fsInput, inout czm_modelMaterial material) {
                // Convert ECEF world position to approximate geodetic lon/lat (degrees).
                // Spherical atan2 — error vs true geodetic is <0.2° max, imperceptible
                // at building scale.
                vec3  p   = fsInput.attributes.positionWC;
                float lon = degrees(atan(p.y, p.x));
                float lat = degrees(atan(p.z, sqrt(p.x * p.x + p.y * p.y)));

                // Map lon/lat into the satellite tile's UV space.
                // u: west→east, v: south→north (matches OpenGL origin after flipY).
                float u = clamp((lon - u_west)  / (u_east  - u_west),  0.0, 1.0);
                float v = clamp((lat - u_south) / (u_north - u_south), 0.0, 1.0);

                material.diffuse = texture(u_sat, vec2(u, v)).rgb;
            }
        `
    });
