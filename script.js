...
const settings = {
    ...
    cameraFollowDelay: 1.0, // 0.2 (snappy/instant) – 5.0 (floaty/laggy) — scales the chase-cam spring rate
...
let camSmoothHeading = null;   // number (radians) | null
let camSmoothHeight  = null;   // number (meters) | null — plane 3rd-person only
...
function updateCesiumCamera(dt) {
    ...
    if (settings.cesiumCameraMode === 'third') {
        ...
        // How quickly the smoothed heading/position/height chase the
        // vehicle. Lower = more lag (dreamier); higher = snappier. dt-scaled
        // so frame-rate changes don't affect the feel. settings.cameraFollowDelay
        ...
        const headingAlpha  = 1.0 - Math.exp(-(6 / followDelay) * dt);   // left/right (heading) spring
        ...
        if (isPlane) {
            // ── Plane: per-axis delay ──────────────────────────────────────
            const heightAlpha = 1.0 - Math.exp(-(8 / followDelay) * dt);   // up/down spring

            // Smooth ONLY the altitude — never the horizontal position.
            ...
            // The anchor uses the plane's REAL current lat/lng (no lag —
            // this is what keeps forward/back distance fixed) but a
            // LAGGED height. Translating the anchor vertically doesn't
            // touch the back/up rotation below at all (heading rotation
            // only spins around the vertical axis), it purely delays the
            // camera's altitude.
            ...
            camSmoothPos = camPos; // kept in sync in case the mode/vehicle is switched mid-flight
        } else {
            // ── Cars/buses: unchanged — uniform world-position lerp ───────
            const posAlpha = 1.0 - Math.exp(-(8 / followDelay) * dt);   // position spring

            const smoothHpr = new Cesium.HeadingPitchRoll(camSmoothHeading, 0, 0);
            const smoothTransform = Cesium.Transforms.headingPitchRollToFixedFrame(vehiclePosition, smoothHpr);
            ...
            if (camSmoothPos === null) {
                camSmoothPos = targetCamPos.clone();
            } else {
                camSmoothPos = Cesium.Cartesian3.lerp(camSmoothPos, targetCamPos, posAlpha, camSmoothPos);
            }
            camPos = camSmoothPos;
        }
        ...
        // "up" in world space = the ENU up at the vehicle position
