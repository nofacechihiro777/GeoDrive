...
let _flashlightLight = null; // Cesium.PointLight, created lazily, reused every frame
...
function toggleMotionBlur(val) {
    ...
    if (val && !_motionBlurStage) {
        _motionBlurStage = new Cesium.PostProcessStage({
            fragmentShader: `uniform sampler2D colorTexture; varying vec2 v_textureCoordinates;
                void main(void){
...
function updateCesiumCamera(dt) {
    ...
    if (settings.cesiumCameraMode === 'third') {
        ...
        if (flashlightOn) {
            if (!_flashlightLight) {
                _flashlightLight = new Cesium.PointLight({
                    color: Cesium.Color.fromCssColorString('#fff3d6'),
                    intensity: 6.0
                });
            }
            _flashlightLight.position = Cesium.Cartesian3.clone(
                cesiumViewer.camera.position, _flashlightLight.position
            );
