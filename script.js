...
// V_r — rotation speed (knots). Below this, the aircraft is still on its
...
function updateFlight(dt) {
    ...
    // ── THRUST & DRAG MODEL ─────────────────────────────────────────────────
    // Tuned for ~350 kts in 2 min at 100% throttle (terminal velocity ~385 kts).
    const maxThrust = 420; // raised from 120
    ...
    const dragCoeff = 0.003; // lowered from 0.012; sets terminal ≈ 385 kts
    const inducedDrag = state.speed * state.speed * dragCoeff;
    const parasiteDrag = flight.gearDown ? 35 : 0; // scaled up with new thrust
    const brakeDrag = flight.brakeActive ? 200 : 0; // scaled up with new thrust

    const netForce = thrust - inducedDrag - parasiteDrag - brakeDrag;
    state.speed += netForce * 0.013 * dt; // lowered from 0.04; smooths the ramp-up curve
    state.speed = Math.max(0, state.speed);

    // ── STALL MODEL ────────────────────────────────────────────────────────
    const stallSpeed = 50;
    const isStalled = state.speed < stallSpeed && flight.alt > 25;
    const currentSpeedMps = state.speed * 0.514444;  // knots → m/s (used in both branches)
...
function updateHighSpeedOptimizations(dt, speedKts, altFt, nowMs) {
    ...
    // ── 2. DYNAMIC FAR-CLIP PLANE & FOG ─────────────────────────────────
    // Far clip scales with altitude (ground = 50 km, 10 km alt = 300 km)
    // then shrinks with speed near the ground so distant tiles never load.
    // Matching fog density hides the pop-out edge naturally.
    const altM    = altFt * 0.3048;
    const altT    = Math.min(1.0, altM / 10000);                      // 0 at ground → 1 at 10 km
    const speedT  = Math.min(1.0, speed / 400);

    const baseFar       = 50000 + altT * 250000;                      // 50 km → 300 km
    const speedCutback  = speedT * 0.55 * (1.0 - altT);              // only cuts near ground
    const targetFar     = Math.max(30000, baseFar * (1.0 - speedCutback));

    const frustum = cesiumViewer.camera.frustum;
    if (frustum && typeof frustum.far !== 'undefined') {
        if (_hsoCurrentFar === null) _hsoCurrentFar = frustum.far;
        _hsoCurrentFar += (targetFar - _hsoCurrentFar) * Math.min(1.0, dt * 2.0);
        frustum.far = _hsoCurrentFar;
    }
