...
function diagnosePhotorealError(e) {
    ...
    if (statusCode === 400 || /API key not valid/i.test(msg)) {
        return '400 Bad Request — Google rejected the key Cesium Ion uses internally for this feature. This usually means Google Photorealistic 3D Tiles hasn\'t been enabled on this Ion account yet (log in at cesium.com → your Ion account and look for a "Google Photorealistic 3D Tiles" entitlement to accept), not that the token you pasted above is wrong.';
    }
...
function tryLoadPhotorealisticTiles() {
    ...
    try {
        // Cesium 1.126 removed the old createGooglePhotorealistic3DTileset(key)
        // positional-string-key call shape — it now only accepts an options
        // object. Using the old shape on a current Cesium build doesn't throw
        // outright, but silently mis-parses the argument, which is one way to
        // end up with a tileset that streams geometry fine while its texture
        // requests never get the right credentials. Always use the options
        // object now, on both the free Ion-proxy path and the own-key path.
        if (CONFIG.PHOTOREAL_KEY && typeof Cesium.GoogleMaps !== 'undefined') {
            Cesium.GoogleMaps.defaultApiKey = CONFIG.PHOTOREAL_KEY;
        }
        const tilesetPromise = Cesium.createGooglePhotorealistic3DTileset({
            onlyUsingWithGoogleGeocoder: true   // silences a one-time console warning; we don't use the Google geocoder
        });
...
async function applyOsmBuildingsSatelliteTexture(tileset) {
    ...
    try {
        const zoom    = 18;  // ~0.6m/px — building-level resolution
        const { x, y } = latLngToOsmTile(state.lat, state.lng, zoom);
        const b       = osmTileToBounds(x, y, zoom);
        const tileUrl = `https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/${zoom}/${y}/${x}`;

        // Pre-fetch the tile as a blob URL so TextureUniform never has to deal
        // with cross-origin restrictions on the remote hostname.
        let textureUrl = tileUrl;
        try {
            const resp = await fetch(tileUrl, { mode: 'cors' });
            if (resp.ok) textureUrl = URL.createObjectURL(await resp.blob());
        } catch (fetchErr) {
            console.warn('Satellite tile pre-fetch failed, falling back to direct URL:', fetchErr);
        }
        ...
        const shaderOptions = {
            ...
            fragmentShaderText: `
                void fragmentMain(FragmentInput fsInput, inout czm_modelMaterial material) {
                    ...
                    // ECEF → WGS84 geodetic lon/lat (degrees) using Bowring's
                    // closed-form. The naive geocentric atan(z, rho) is off by
                    // ~0.19° in latitude at 33°S — roughly 30 tile-widths at
                    // zoom 16 — which clamps every fragment to the tile edge.
                    ...
                    // v is flipped: ArcGIS tiles have north at image row 0; after WebGL
                    // flipY that maps to v=1, so south→v=1 and north→v=0 in the sampler.
                    float v = 1.0 - clamp((lat - u_south) / (u_north - u_south), 0.0, 1.0);

                    vec4 sat = texture(u_sat, vec2(u, v));
                    material.diffuse = sat.rgb;
                    material.alpha   = 1.0;
                }
            `
        };
