...
function _setNightImageryDimming(on) {
    ...
    for (let i = 0; i < layers.length; i++) {
        ...
        if (!layer) continue;
        layer.brightness = on ? 0.18 : 1.0;
        layer.contrast   = on ? 1.1  : 1.0;
        layer.gamma       = on ? 0.55 : 1.0;
    }
}
...
function _applyDayNightMode() {
    ...
    if (isNightMode) {
        ...
        cesiumViewer.scene.light = new Cesium.DirectionalLight({
            ...
            intensity: 0.05  // very dim — moonlight level
        });
...
function _spawnNightLights() {
    ...
    for (let i = 0; i <= NUM_LIGHTS; i++) {
        ...
        const elevAtPoint = (leElevM != null && heElevM != null)
            ? leElevM + (heElevM - leElevM) * t
            : (getGroundHeight(cLng, cLat) || 0);
    ...
    const leElevForPapi = leElevM != null ? leElevM : (getGroundHeight(leLng, leLat) || 0);
    const heElevForPapi = heElevM != null ? heElevM : (getGroundHeight(heLng, heLat) || 0);
