...
function toggleSettings() {
    ...
    if (opening) updateSettingsTabStates();
}

// ==========================================
// SETTINGS MODAL — TABBED LAYOUT
// Left sidebar of category buttons, right-hand content panel. Categories
// that don't apply to the current vehicle/map style (✈️ Airplane while
// driving, 🏙️ GP3DT while the Terrain+OSM style is active) are greyed out
// and disabled rather than removed, so the layout stays stable as the
// player switches vehicles/modes.
// ==========================================
let _activeSettingsTab = 'display';

function switchSettingsTab(tab) {
    const tabBtn = document.querySelector(`.settings-tab[data-tab="${tab}"]`);
    if (tabBtn && (tabBtn.disabled || tabBtn.classList.contains('tab-disabled'))) return;
    ...
    // Scroll the newly-shown panel back to the top rather than leaving it
    // wherever the previous tab's scroll position happened to be.
    const panels = document.getElementById('settings-panels');
    if (panels) panels.scrollTop = 0;
}

/**
 * updateSettingsTabStates — grays out/disables category tabs that don't
 * apply to the current vehicle or map style, and bounces the user off of a
 * tab that just became unavailable (e.g. switching from plane → car while
 * the Airplane tab is open). Called on open, and whenever vehicle or
 * mapStyle changes (vehicle-select listener, set3DStyle()).
 */
function updateSettingsTabStates() {
    const airplaneTab = document.getElementById('tab-btn-airplane');
    const gp3dtTab     = document.getElementById('tab-btn-gp3dt');

    const isPlane     = state.vehicle === 'plane';
    const isPhotoreal = settings.mapStyle === 'photoreal';

    if (airplaneTab) {
        airplaneTab.disabled = !isPlane;
        airplaneTab.classList.toggle('tab-disabled', !isPlane);
        if (!isPlane && _activeSettingsTab === 'airplane') switchSettingsTab('display');
    }
    if (gp3dtTab) {
        gp3dtTab.disabled = !isPhotoreal;
        gp3dtTab.classList.toggle('tab-disabled', !isPhotoreal);
        if (!isPhotoreal && _activeSettingsTab === 'gp3dt') switchSettingsTab('display');
    }
}
...
function buildCloudSkyBox() {
    ...
    cloudSkyBoxPromise = new Promise((resolve, reject) => {
        ...
        img.onload = () => {
            const CELL = img.width / 4; // 300px at native 1200×900
            // [col, row] within the 4×3 cross for each cube face
            const layout = {
                positiveZ: [1, 0], // top    → up
                negativeZ: [1, 2], // bottom → down
                negativeX: [0, 1], // left
                positiveX: [1, 1], // front
                positiveY: [2, 1], // right
                negativeY: [3, 1]  // back
            ...
            resolve(new Cesium.SkyBox({ sources }));
        };
        img.onerror = reject;
        img.src = 'skybox.png';
    });
...
function updateSkyBox(mode) {
    ...
    if (mode === 'default') {
        if (cesiumDefaultSkyBox) cesiumViewer.scene.skyBox = cesiumDefaultSkyBox;
        return;
    }
    ...
    buildCloudSkyBox().then(skyBox => {
        if (settings.skyBox !== 'clouds' || !cesiumViewer) return; // user changed their mind mid-load
...
function confirmSpawnLocation() {
    ...
    const lat = _spawnSelected.lat;
    const lng = _spawnSelected.lng;
    const elev = _spawnSelected.elev; // feet (may be null)

    // ── 1. Apply selected spawn position to game state ─────────────────────
    state.lat = lat;
    state.lng = lng;

    // For plane: if we have elevation data, start just above runway level
    if (state.vehicle === 'plane' && elev != null) {
        // elev is in feet; flight.alt is feet AGL relative to groundRef.
        // We set groundRef to the elevation so the plane spawns ON the runway.
        flight.groundRef = elev * 0.3048; // convert ft to metres for Cesium
    ...
    // Reset heading to runway-ish north
    state.heading = 0;
    state.speed   = 0;
    ...
    // This prevents the camera from briefly appearing at the previous position.
    if (cesiumViewer) {
        try {
            const spawnAlt   = elev != null
                ? Math.max(elev * 0.3048+ 50, 200)  // 50 m above runway or min 200 m
                : 500; // default 500 m if no elevation
            const pos = Cesium.Cartesian3.fromDegrees(lng, lat, spawnAlt + 10);
            cesiumViewer.camera.setView({
                destination: pos,
                orientation: {
    ...
    setTimeout(() => {
        ...
        // Replaces the old fixed delay: the loading screen now stays up
        // until the radius around the spawn point has actually streamed in
        // (terrain/buildings/3D tiles, or map tiles in CSS/2D mode), so
        // there's no further pop-in/loading hitch once the player appears.
