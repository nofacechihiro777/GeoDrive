...
const settings = {
    ...
    renderMode: '3D',   // '3D' = Cesium (first-person only), 'CSS' = CSS perspective (chase cam), '2D' = flat
    tilt: 0,
...
function setRenderMode(mode) {
    ...
    if (mode === '3D' && cesiumViewer) {
        // ── Cesium mode (first-person, no 2D canvas at all) ────────────────
...
function updateCesiumCamera() {
    ...
    // Eye height above the ellipsoid. Ground vehicles use a fixed
    // approximate driver eye height (this app uses flat/sea-level terrain
    // by default, same simplification the rest of the physics already
    // makes). The plane uses its real, simulated flight altitude.
    const eyeHeight = isPlane
        ? flight.alt * 0.3048                              // ft → m
        : (state.vehicle === 'bus' ? 2.4 : 1.3);            // bus driver sits higher than a car
    ...
    const camRoll      = isPlane ? flight.roll : 0;          // radians, direct from flight.roll
