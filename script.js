...
function diagnosePhotorealError(e) {
    ...
    if (statusCode === 403 || /\b403\b/.test(msg)) {
        return '403 Forbidden from Cesium ion — the Ion token above may be invalid, revoked, or missing the right scope.';
    }
    if (statusCode === 404 || /\b404\b/.test(msg)) {
        return '404 Not Found — try re-saving the Ion token.';
    }
    if (statusCode === 400 || /API key not valid/i.test(msg)) {
        return '400 Bad Request — Google Photorealistic 3D Tiles hasn\'t been enabled on this Ion account yet.';
    }
