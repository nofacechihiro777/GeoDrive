...
function updateFlight(dt) {
    // Si JSBSim está disponible, usarlo
    if (window.JSBSim) {
        // Inicializar FDM si no existe
        if (!window.jsbsimFDM) {
            try {
                window.jsbsimFDM = window.JSBSim.createFDM();
                window.jsbsimFDM.loadModel('c172p');
                window.jsbsimFDM.setPropertyValue('position/lat-gc-deg', state.lat);
                window.jsbsimFDM.setPropertyValue('position/long-gc-deg', state.lng);
                window.jsbsimFDM.setPropertyValue('position/h-sl-ft', 20);
                console.log('✅ JSBSim FDM loaded');
            } catch (e) {
                console.error('❌ JSBSim FDM init failed:', e);
            }
        }

        const fdm = window.jsbsimFDM;
        if (fdm) {
            try {
                // Actualizar controles
                fdm.setPropertyValue('fcs/elevator-cmd-norm', flightInput.pitch);
                fdm.setPropertyValue('fcs/aileron-cmd-norm', flightInput.roll);
                fdm.setPropertyValue('fcs/throttle-cmd-norm', flight.throttle / 100);
                fdm.setPropertyValue('fcs/flap-cmd-norm', flight.flaps / 2);
                fdm.setPropertyValue('gear/gear-cmd-norm', flight.gearDown ? 1 : 0);

                // Correr simulación
                fdm.run();

                // Leer resultados
                state.lat = fdm.getPropertyValue('position/lat-gc-deg');
                state.lng = fdm.getPropertyValue('position/long-gc-deg');
                flight.alt = fdm.getPropertyValue('position/h-sl-ft');
                state.speed = fdm.getPropertyValue('velocities/vc-kts');
                flight.verticalSpeed = fdm.getPropertyValue('velocities/v-down-fps') * 60;
                flight.pitch = fdm.getPropertyValue('attitude/theta-rad');
                flight.roll = fdm.getPropertyValue('attitude/phi-rad');
                state.heading = (fdm.getPropertyValue('attitude/psi-rad') * 180 / Math.PI + 360) % 360;

                return;
            } catch (e) {
                console.error('JSBSim update error:', e);
            }
        }
    }

    // FÍSICA ORIGINAL (fallback si JSBSim no está disponible)
...
function tryApplyGP3DTSettingsOnLoad() {
    ...
    setGP3DTPreset((saved && GP3DT_PRESETS[saved]) ? saved : 'normal');
}
...
// Pre-computed 180° rotation around Z (vehicle up-axis) applied to GLB
// models every frame so the airplane faces the correct direction.
const _scratchGlbRot180 = Cesium.Matrix4.fromRotationTranslation(
...
function updateVehicleModels(vehicleTransform) {
    ...
    Object.keys(vehiclePrimitives).forEach(type => {
        ...
        for (let i = 0; i < parts.length; i++) {
            ...
            // Apply 180° yaw rotation for GLB models (a320.glb faces backward
            // by default in Cesium's ENU frame; this corrects it without
            // touching forwardAxis/upAxis in VEHICLE_MODEL_DEFS).
            if (prim._isGlb) {
                Cesium.Matrix4.multiply(_scratchModelM, _scratchGlbRot180, _scratchModelM);
            }
...
function update() {
    ...
    if (cesiumViewer && state.vehicle === 'plane') {
        ...
        // 1.5 m tolerance absorbs minor GP3DT mesh bumps so we don't
        // falsely trigger on flat terrain where the raycast has sub-metre
        // variance frame-to-frame.
        if (absoluteHeightMeters < currentGround - 1.5) {
