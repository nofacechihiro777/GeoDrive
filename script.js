...
const VEHICLE_MODEL_DEFS = {
    ...
    //
    // ROTATION FIX (confirmed via screenshot — car was sitting "sideways"
    // relative to its direction of travel, not front/back-on): the wheels
    // were already touching the ground correctly, so upAxis:'Y' is right;
    // only the forward axis was wrong. forwardAxis:'Z' showed the car's
    // side profile to the chase camera — a 90° error, not a 180° one — so
    // NEGATIVE_Z (which would only flip nose↔tail, still front/back-on)
    // wasn't the right next step. Z and X are the two horizontal axes
    // perpendicular to Y-up, so swapping to the other one is exactly a 90°
    // turn, which is what the symptom called for. This mirrors the same
    // fix that was already verified for a320_cockpit.glb below (forwardAxis
    // 'Z' → 'X' fixed an identical 90°-off symptom there).
    // If it's now reversed (front of car points where the back should be),
    // try 'NEGATIVE_X' next. If it's still showing the side profile, the
    // model wasn't actually off by 90° around Y and 'NEGATIVE_Z' (180°,
    // the original guess) is worth trying after all.
    car: {
        type: 'glb',
        url: 'car.glb',
        scale: 0.182,
...
// Same base method as a320_cockpit.glb above: discard the glass
// fragments entirely so the window reads as a clear opening
// instead of a solid panel (rather than blending it, which is
// what produced the see-through-fuselage bug in the first place).
//
// The difference: I don't have car.glb here to inspect its actual
// material setup, so I can't confirm whether its windows use
// alphaMode:"BLEND" (alpha < 1) like the plane's do, or whether
// they're baked as a fully OPAQUE black material instead (common in
// car-configurator exports that fake "tinted glass" with a dark,
// glossy material instead of real alpha transparency) — which is
// what the screenshot's solid black windows suggest is happening.
// This shader covers BOTH cases:
//   1) Same exact check as the plane (alpha < 0.5 → discard), in
//      case the windows DO use alpha blending.
//   2) A fallback heuristic for case (2): a fragment whose diffuse
//      colour is very dark AND whose surface is very smooth/glossy
//      (low roughness) gets discarded too. Tinted automotive glass
//      is reliably dark+glossy; matte black trim, tyres, and most
//      black plastic are dark but ROUGH, so they're left alone.
// u_carGlassDarkness / u_carGlassRoughness are exposed as uniforms
// (not hard-coded) specifically so they're easy to retune from the
// console without touching the shader string — see the comment
// just below the shader for how.
const _glbOpaqueShaderCar = new Cesium.CustomShader({
    lightingModel: Cesium.LightingModel.PBR,
    translucencyMode: Cesium.CustomShaderTranslucencyMode.OPAQUE,
    uniforms: {
        u_carGlassDarkness: { type: Cesium.UniformType.FLOAT, value: 0.06 },
        u_carGlassRoughness: { type: Cesium.UniformType.FLOAT, value: 0.25 }
    },
    fragmentShaderText: [
        'void fragmentMain(FragmentInput fsInput, inout czm_modelMaterial material) {',
        '    if (material.alpha < 0.5) {',
        '        discard;',
        '    }',
        '    float _luma = dot(material.diffuse, vec3(0.299, 0.587, 0.114));',
        '    if (_luma < u_carGlassDarkness && material.roughness < u_carGlassRoughness) {',
        '        discard;',
        '    }',
        '    material.alpha = 1.0;',
        '}'
...
// Tuning, if the heuristic above over/under-shoots on the real car.glb:
//   - Rims/trim/mirrors turn transparent too  → LOWER u_carGlassRoughness.value
//     (e.g. 0.25 → 0.12) so only the glossiest surfaces qualify.
//   - Windows stay solid black                → RAISE u_carGlassDarkness.value
//     (e.g. 0.06 → 0.12) so slightly-less-pure-black materials still match.
// From the browser console (after the car model is loaded):
//   vehiclePrimitives.car[0].customShader.uniforms.u_carGlassRoughness.value = 0.12;
...
function updateCesiumCamera(dt) {
    ...
    if (settings.cesiumCameraMode === 'third') {
        ...
        if (isPlane) {
            ...
            //
            // freeLook orbit: the default offset (0, -back, up) is expressed
            // in spherical coordinates so yaw/pitch rotate the camera around
            // the vehicle without changing the orbit radius.
